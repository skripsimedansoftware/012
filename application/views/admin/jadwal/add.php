...
<section class="content-header">
	<h1>Tambah Data<small>Jadwal Praktek</small></h1>
</section>
...
<section class="content container-fluid">
	<div class="box">
		...
		<form method="POST" action="<?= base_url($this->router->fetch_class().'/jadwal/add') ?>">
		<div class="box-body">
			<div class="row">
				<div class="col-lg-6">
					<div class="form-group">
						...
						<?php foreach ($this->user->read(array('role' => 'pasien'))->result() as $pasien):?>
							<option value="<?= $pasien->id ?>" <?= set_value('pasien') == $pasien->id ? 'selected':'' ?>><?= $pasien->full_name ?></option>
						<?php endforeach; ?>
						</select>
					...
					<div class="form-group">
						...
						<?php foreach ($this->user->read(array('role' => 'dokter'))->result() as $dokter):?>
							<option value="<?= $dokter->id ?>" <?= set_value('dokter') == $dokter->id ? 'selected':'' ?>><?= $dokter->full_name ?></option>
						<?php endforeach; ?>
						</select>
					...
					<div class="form-group">
						<label>Waktu</label>
						<input class="form-control datemask" type="text" name="waktu" placeholder="Waktu" value="<?= set_value('waktu') ?>">
						<?php echo form_error('waktu', '<span class="help-block error">', '</span>'); ?>
					</div>
					<div class="form-group">
						<label>Catatan</label>
						<textarea name="catatan" class="form-control" placeholder="Catatan..."><?= set_value('catatan') ?></textarea>
