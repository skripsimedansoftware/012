	...
	<div class="box">
		...
		<div class="box-body">
			...
			<table class="table table-hover table-striped datatable">
				...
				<tbody>
					...
					<tr>
						...
						<td><?= $data->catatan ?></td>
						<td><?= ($data->status == 'dijadwalkan') ? $data->status : '<a href="'.base_url($this->router->fetch_class().'/print_report/'.$data->pasien).'">Lihat Laporan</a>' ?></td>
						<td>
							<?php
							if ($data->status !== 'dijadwalkan')
							{
								...
								{
									echo '<a href="'.base_url($this->router->fetch_class().'/rekam_medis/add').'">Buat Rekam Medis</a>';
								}
								else
								{
									echo '<a href="'.base_url($this->router->fetch_class().'/print_report/'.$data->pasien).'">Lihat Laporan</a>';
								}
							}
							else
							{
			...
			<table class="table table-hover table-striped datatable">
				...
				<tbody>
					<?php foreach ($this->jadwal->read(array('dokter' => $user->id))->result() as $key => $data) : ?>
					<tr>
						<td><?= $key+1 ?></td>
						<td><?= $this->user->read(array('id' => $data->pasien))->row()->full_name ?></td>
						<td><?= $this->user->read(array('id' => $data->dokter))->row()->full_name ?></td>
